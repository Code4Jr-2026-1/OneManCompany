...
export default async function CoachHome() {
  ...
  // Live this-month billing (private), matching /coach/billing
  ...
  for (const s of students) {
    for (const ss of s.scheduledSessions) {
      upcomingItems.push({ kind: "private", date: new Date(ss.scheduledAt), studentId: s.id, studentName: s.name, skillLevel: s.skillLevel, duration: ss.duration })
    }
  }
  ...
  const upcoming = upcomingItems.slice(0, 8)
  const sessionsThisWeek = upcomingItems.length
          ...
          <div className="lg:col-span-2">
            <div className="bg-card border border-border rounded-xl shadow-sm">
              <div className="px-6 py-4 border-b border-border flex items-center justify-between">
                <h2 className="font-semibold text-card-foreground">Upcoming Sessions</h2>
              ...
              {upcoming.length === 0 ? (
                ...
                <div className="divide-y">
                  {upcoming.map((item, i) => {
                    const isToday = item.date >= todayStart && item.date < new Date(todayStart.getTime() + 86400000)
                    if (item.kind === "group") {
                      return (
                        <div key={`g-${item.id}-${i}`} className="px-6 py-4 flex items-center gap-4">
                          <div className="w-20 text-xs text-muted-foreground flex-shrink-0">
                            <div>{isToday ? "Today" : fmt(item.date)}</div>
                          ...
                          {isToday ? (
                            <>
                              <Link href={`/coach/students/${item.studentId}`}>
                                <button className="text-sm bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700">📋 Profile</button>
                              </Link>
                              <Link href={`/coach/students/${item.studentId}/end-session`}>
            ...
            <div className="bg-card border border-border rounded-xl shadow-sm p-5">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Billed this month</span>
            ...
            <div className="bg-card border border-border rounded-xl shadow-sm p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Link href="/coach/students" className="rounded-lg border border-border p-3 hover:bg-accent transition-colors">
                  <Users className="size-4 text-blue-600 mb-1.5" aria-hidden="true" />
                ...
                <Link href="/coach/schedule" className="rounded-lg border border-border p-3 hover:bg-accent transition-colors">
                  <CalendarCheck className="size-4 text-blue-600 mb-1.5" aria-hidden="true" />
                  ...
                  {sessionsTrend !== 0 && (
                    <p className={`text-xs flex items-center gap-0.5 mt-0.5 ${sessionsTrend > 0 ? "text-green-600" : "text-amber-600"}`}>
                      {sessionsTrend > 0 ? <TrendingUp className="size-3" aria-hidden="true" /> : <TrendingDown className="size-3" aria-hidden="true" />}
                ...
                <Link href="/coach/students" className={`rounded-lg border p-3 transition-colors ${needsAttention.length > 0 ? "border-amber-200 bg-amber-50 hover:bg-amber-100" : "border-border hover:bg-accent"}`}>
                  <TriangleAlert className={`size-4 mb-1.5 ${needsAttention.length > 0 ? "text-amber-600" : "text-muted-foreground"}`} aria-hidden="true" />
                ...
                <Link href="/coach/homework" className={`rounded-lg border p-3 transition-colors ${homeworkPending > 0 ? "border-amber-200 bg-amber-50 hover:bg-amber-100" : "border-border hover:bg-accent"}`}>
                  <BookOpen className={`size-4 mb-1.5 ${homeworkPending > 0 ? "text-amber-600" : "text-muted-foreground"}`} aria-hidden="true" />
                  <p className={`text-lg font-bold ${homeworkPending > 0 ? "text-amber-700" : "text-foreground"}`}>{homeworkPending}</p>
                  <p className="text-xs text-muted-foreground">Homework pending</p>
                </Link>
              </div>
            </div>
