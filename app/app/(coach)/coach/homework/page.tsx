...
import { HomeworkActions, AddHomeworkButton } from "./actions"
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Homework Tracker</h1>
            <p className="text-muted-foreground text-sm mt-1">{pending.length} pending · {done.length} completed</p>
          </div>
          <div className="flex gap-2">
            {students.map(s => (
              <AddHomeworkButton key={s.id} studentId={s.id} studentName={s.name} />
            ))}
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl shadow-sm mb-6">
          ...
          {pending.length === 0 ? (
            <p className="px-6 py-6 text-sm text-muted-foreground">All homework complete! 🎉</p>
          ) : (
              ...
              {pending.map(hw => (
                <div key={hw.id} className="px-6 py-4 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">{hw.studentName} · {hw.dueDate ? `Due ${new Date(hw.dueDate).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}` : "No due date"}</p>
        ...
        <div className="bg-card border border-border rounded-xl shadow-sm">
          ...
          {done.length === 0 ? (
            <p className="px-6 py-6 text-sm text-muted-foreground">No completed homework yet.</p>
          ) : (
            <div className="divide-y">
              {done.map(hw => (
                <div key={hw.id} className="px-6 py-3 flex items-center gap-4 opacity-60">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-foreground text-sm line-through">{hw.title}</p>
                    <p className="text-xs text-muted-foreground">{hw.studentName} · {hw.completedAt ? `Completed ${new Date(hw.completedAt).toLocaleDateString()}` : "Done"}</p>
